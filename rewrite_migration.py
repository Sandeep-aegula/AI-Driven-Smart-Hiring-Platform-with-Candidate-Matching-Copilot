content = '''"""Migrate all data from storage.json to MySQL."""
...
async def migrate_json_to_mysql() -> None:
    ...
    print(
        f"Loaded data: {len(skills_data)} skills, {len(jobs_data)} jobs, "
        f"{len(candidates_data)} candidates, {len(applications_data)} applications, "
        f"{len(resume_data)} resumes, {len(interviews_data)} interviews, "
        f"{len(employees_data)} employees, {len(activities_data)} activities"
    )
                        ...
                        try:
                            setattr(
                                ...
                                datetime.fromisoformat(
                                    job_json[field].replace("Z", "+00:00")
                                ),
                            )
                        except Exception:
                            pass
                    ...
                    if cand_json.get(field):
                        try:
                            setattr(
                                ...
                                field,
                                datetime.fromisoformat(
                                    cand_json[field].replace("Z", "+00:00")
                                ),
                            )
                        except Exception:
                            pass
                        ...
                        if app_json.get(field):
                            try:
                                setattr(
                                    ...
                                    field,
                                    datetime.fromisoformat(
                                        app_json[field].replace("Z", "+00:00")
                                    ),
                                )
                            except Exception:
                                pass
            ...
            if not resume:
                ...
                if cand_id:
                    ...
                    if resume_json.get("created_at"):
                        try:
                            resume.created_at = datetime.fromisoformat(
                                resume_json["created_at"].replace("Z", "+00:00")
                            )
                        except Exception:
                            pass
                    session.add(resume)
        ...
        for emp_json in employees_data:
            ...
            if not emp:
                ...
                emp = Employee(
                    ...
                    reporting_manager=emp_json.get(
                        "reporting_manager", emp_json.get("manager", "")
                    ),
                    ...
                    if emp_json.get(field):
                        try:
                            setattr(
                                ...
                                field,
                                datetime.fromisoformat(
                                    emp_json[field].replace("Z", "+00:00")
                                ),
                            )
                        except Exception:
                            pass
        ...
        for act_json in activities_data:
            ...
            if not act:
                ...
                if act_json.get("created_at"):
                    try:
                        act.created_at = datetime.fromisoformat(
                            act_json["created_at"].replace("Z", "+00:00")
                        )
                    except Exception:
                        pass
                session.add(act)
...
with open('backend/database/migrate_json_to_mysql.py', 'w', encoding='utf-8') as f:
    f.write(content)
print('Migration script rewritten with proper formatting')
