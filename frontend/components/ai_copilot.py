...
def _get_conn() -> sqlite3.Connection:
    ...
    conn.execute(
        ...
        CREATE TABLE IF NOT EXISTS resume_context (
            ...
            file_text TEXT NOT N"""AI Copilot page.
...
def _restore_persisted_resume(session_id: str) -> None:
    """On this session's first run, re-attach any previously saved resume
    via the existing attach_resume_context() API, so a rerun that clears
    copilot_service's own state doesn't lose the resume the recruiter can
    chat about."""
    if st.session_state.get("_resume_restored"):
        return
    if not get_resume_context():
        saved = _load_persisted_resume(session_id)
        if saved:
            attach_resume_context(saved["file_name"], saved["file_text"])
            st.session_state["resume_uploaded"] = True
            st.session_state["resume_name"] = saved["file_name"]
            st.session_state["resume_text"] = saved["file_text"]
...
def _handle_resume_upload(uploaded_file) -> bool:
    ...
    _save_persisted_resume(_get_session_id(), uploaded_file.name, file_text)
    st.session_state["resume_uploaded"] = True
...
def render_ai_copilot() -> None:
    ...
    _restore_persisted_resume(session_id)
        ...
        with st.container(border=True):
            ...
            if resume_uploaded and resume_name:
                ...
                if st.button(
                    ...
                    clear_resume_context()
                    _clear_persisted_resume(session_id)
                    st.session_state["_resume_restored"] = False
                    st.rerun()
            else:
